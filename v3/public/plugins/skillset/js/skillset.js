(function ( $ ) {
	...
	$.fn.skillset = function( options ) {
		...
		function bar_loop(start,value,length,color){
			...
			for(var i = 0; i < length; i++){
				...
				setTimeout(function(){
					...
					if(j < value){
						var html = '<div class="full" style="background-color:' + color + '"></div>';
		...
		function create_trigger(element){
			...
			for(var i = 0; i < count; i++){
				...
				bar_loop(html,value,length,settings.object[i]['color']);
